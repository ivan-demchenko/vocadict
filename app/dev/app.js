...
define('text!templates/landing.html',[],function () { return '\n<div id="landing-banner">\n  <h1>Welcome to Vocadict!</h1>\n  <p>Here tou can find similar music with help of VK.com and LastFM</p><a href="/joinvk">Join us with VK.com</a>\n</div>';});

define('appLogic', ['jquery', 'underscore', 'backbone', 'soundManager'], function($, _, Backbone, SoundMan) {
  var appLogic;
  ...
  return appLogic = {
    ...
    log: function() {
      if (appLogic.debug && (window.console != null)) {
    ...
    init: function() {
      require(['views/layout'], function(Layout) {
        ...
        appLogic.trigger('list.load', {
          type: 'my'
        });
        return SoundMan.setup({
          url: '/vendor/soundmanager/soundmanager2.swf',
          onready: function() {
            if (appLogic.player.man === null) {
              appLogic.player.man = SoundMan;
            }
            return require(['views/player/playerView'], function(PlayerView) {
              return appLogic.views.player = new PlayerView();
            });
          }
        });
      });
    ...
    methods: {
      ...
      messages: {
        ...
        hide: function() {
          if (appLogic.methods.messages.timer != null) {
            clearTimeout(appLogic.methods.messages.timer);
          }
          appLogic.methods.messages.timer = setTimeout(function() {
      ...
      currentTrack: {
        current: null,
        set: function(data) {
          if (appLogic.methods.currentTrack.current !== data.view) {
            if (appLogic.methods.currentTrack.current != null) {
      ...
      playTrack: function(data) {
        ...
        if (data.url != null) {
          if (appLogic.player.currSong !== null) {
            appLogic.player.currSong.destruct();
          }
          appLogic.player.currSong = appLogic.player.man.createSound({
            ...
            whileloading: function() {
              return appLogic.views.player.setLoadData(this);
            },
            whileplaying: function() {
              return appLogic.views.player.setPlayingProgress(this);
            },
            onfinish: function() {
              return appLogic.views.player.playingFinished(this);
            }
          });
        ...
        require(['collections/vkSongs'], function(SongsCollection) {
          ...
          vkSongs.fetch({
            ...
            success: function(collection) {
              ...
              return appLogic.trigger('list.loaded', {
                data: params,
                collection: collection
              });
            }
          });
          return appLogic.collections.push(vkSongs);
        });
        return this;
      },
        ...
        require(['collections/vkSongs'], function(SongsCollection) {
          ...
          vkSongs.fetch({
            ...
            error: function(jqXHR, textStatus, errorThrown) {
              return appLogic.log(jqXHR, textStatus, errorThrown);
            },
            success: function(collection, response) {
              if (response.error) {
                appLogic.messages.auto(response.error.error_msg);
              }
              return appLogic.trigger('list.loaded', {
                data: params,
                collection: collection
          ...
          collection.fetch({
            ...
            success: function(collection, response) {
              if ((response.similartracks != null) && response.similartracks['#text'] === void 0) {
                ...
                return appLogic.methods.messages.auto('red', 'Similar tracks not found.');
              }
            }
          });
          return appLogic.collections.push(collection);
        });
        return this;
      },
      loadList: function(params) {
        ...
        switch (params.type) {
          case "my":
          case "vk":
            appLogic.methods.loadVkTracksList(params);
            break;
          case "lastfm":
          case "search":
            appLogic.methods.loadSimilarTracksList(params);
            break;
          default:
            alert('No spec');
        }
      ...
      showTrackList: function(params) {
        ...
        require(['views/tracksList/' + params.data.type + 'TracksList'], function(ListView) {
          ...
          return appLogic.views[list.cid] = list;
        });
        return this;
      },
...
define('app', ['jquery', 'underscore', 'backbone', 'appLogic'], function($, _, Backbone, appLogic) {
  var app, router;
  ...
  $.ajaxSetup({
    beforeSend: function() {
      return app.methods.messages.show('blue', 'Just a few seconds, loading...');
    },
    complete: function() {
  ...
  router = Backbone.Router.extend({
    ...
    index: function() {
      ...
      return appLogic.start();
    },
    landing: function() {
      return require(['views/landing'], function(LandingLayout) {
        return new LandingLayout({
          el: document.getElementById('app')
        });
      });
    },
    joinvk: function() {
      ...
      url = 'http://oauth.vk.com/authorize?' + 'client_id=' + appLogic.vk.app_id + '&scope=audio,offline' + '&redirect_uri=' + appLogic.rootURL + '/vkauth' + '&display=page' + '&response_type=token';
...
define('views/landing', ['jquery', 'underscore', 'backbone', 'app', 'text!templates/landing.html'], function($, _, Backbone, app, html) {
  var View;
...
define('views/tracksList/lastfmTracksList', ['jquery', 'underscore', 'backbone', 'app', 'views/track/lastfmTrack', 'text!templates/tracksList/lastfmTracksList.html'], function($, _, Backbone, app, TrackView, html) {
  return Backbone.View.extend({
    template: _.template(html),
...
define('views/tracksList/myTracksList', ['jquery', 'underscore', 'backbone', 'app', 'views/track/vkTrack', 'text!templates/tracksList/myTracksList.html'], function($, _, Backbone, app, TrackView, html) {
  return Backbone.View.extend({
    template: _.template(html),
    className: 'tracks-list',
    initialize: function(params) {
...
define('views/tracksList/searchTracksList', ['jquery', 'underscore', 'backbone', 'app', 'views/track/searchTrack', 'text!templates/tracksList/myTracksList.html'], function($, _, Backbone, app, TrackView, html) {
  return Backbone.View.extend({
    template: _.template(html),
    className: 'tracks-list',
    initialize: function(params) {
...
define('text!templates/tracksList/myTracksList.html',[],function () { return '\n<header><span><%= listTitle %></span></header>\n<div></div>';});
