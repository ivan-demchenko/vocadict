...
define([
  ...
  function ($, _, BB, app) {
        ...
        methods: {
            ...
            init: function() {
                ...
                app.trigger('list.load', {
                    type: 'my',
                    $domElement: $('#track-lists-wrapper'),
                    listTitle: 'My Tracklist',
                    title: '',
                    artist: ''
                });
            },
            ...
            loadSimilarTrackList: function loadSimilarTrackList(params) {
                require(['collections/lastfmSongs'], function(LastCollection) {
                    ...
                    collection.fetch({
                        ...
                        success: function(collection, response){
                            if( typeof response.similartracks !== 'undefined' ) {
                                if( typeof response.similartracks['#text'] === 'undefined' ) {
                                    app.trigger('list.loaded', {data: params, collection: collection});
                                } else {
    ...
    return _.extend(window.app, BB.Events);
